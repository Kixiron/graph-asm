...
use crate::{cost::OperatorCost, rewrites::rules, RecExpr, Runner};
                            ...
                            (arrange_by_key
                                (map
                                    (join (arrange_by_self NeedsFibonacci)
                                          (arrange_by_key (map FibonacciLoop (fun (sub #0 1)))))
                                    (fun (case #0 (tuple _ _) (tuple #3 #0)))))
                            (arrange_by_key
                                (map
                                    (join (arrange_by_self NeedsFibonacci)
                                          (arrange_by_key (map FibonacciLoop (fun (sub #0 2)))))
                                    (fun (case #0 (tuple _ _) (tuple #3 #0))))))
